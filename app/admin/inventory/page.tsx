...
export default function InventoryManagerPage() {
  ...
  const [stockRange, setStockRange] = useState<{ min: number; max: number }>({ min: 0, max: Infinity });
  ...
  const refreshProducts = async () => {
    try {
      ...
      if (!productRes.ok || !subcatRes.ok || !catRes.ok) {
        throw new Error(`Fetch failed (${productRes.status}/${subcatRes.status}/${catRes.status})`);
      }
      ...
      const subcategories = Array.isArray(subcategoriesJson) ? subcategoriesJson : [];
      const categories = Array.isArray(categoriesJson) ? categoriesJson : [];
      const productsArr = Array.isArray(productsJson) ? productsJson : [];

      // Build lookup maps
      const subcatMap: Record<string, { name: string; status: string; categories: string[] }> = {};
      ...
      const enriched = productsArr.map((p: any) => {
        ...
        const hasVisibleCategory = subcatCategories.some((catName) => categoryMap[catName] === 'visible');
  ...
  const areAllSelected =
    filteredInventory.length > 0 && filteredInventory.every((p) => selectedProductIds.includes(String(p.id)));
    ...
    <AdminAuthGuard>
      ...
      <div className="flex min-h-screen bg-gradient-to-br from-gray-50 to-white">
        ...
        <main className="flex-1 p-4 sm:p-6">
          {/* Header */}
          <div className="mb-6 bg-gradient-to-r from-white via-[#f8f9fa] to-gray-100 p-6 rounded-2xl shadow-sm border border-gray-200 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <div>
              <h1 className="text-3xl font-bold text-gray-800">📦 Inventory</h1>
              <p className="text-gray-500 mt-1 text-sm">Manage product stock, availability, and pricing.</p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
              <input
                ...
                className="border border-gray-300 px-3 py-1.5 rounded-md text-sm text-black placeholder:text-gray-400"
              />
              <select
                onChange={(e) => {
                ...
                className="border border-[#891F1A] text-[#891F1A] px-3 py-1.5 rounded-md text-sm font-medium bg-white"
              >
          ...
          {/* Table */}
          <div className="overflow-hidden rounded-2xl shadow-lg border border-gray-200 max-h-[500px] overflow-y-auto">
            <table className="w-full table-auto text-sm bg-white">
              <thead className="text-white bg-[#891F1A] sticky top-0 z-10 text-xs">
                <tr>
                  <th className="pl-1 pr-2 py-1.5 text-center w-4">
                    <Checkbox
                  ...
                  <th className="pr-3 py-1.5 text-center">ID</th>
                  <th className="px-2 py-1.5 text-center">Thumbnail</th>
                  <th className="pl-2 pr-1 py-1.5 text-left w-[20%]">Name</th>
                  <th className="pl-1 pr-2 py-1.5 text-center">No. of Items</th>
                  <th className="px-2 py-1.5 text-center">Price</th>
                  <th className="px-2 py-1.5 text-center">Printing</th>
              ...
              <tbody className="text-gray-800 divide-y divide-gray-100">
                {filteredInventory.map((prod) => {
                  const imgSrc = prod.images?.[0]?.value ? toAbsolute(prod.images[0].value) : '/img1.jpg';
                  const printingList = prod.printingMethod ?? prod.printing_methods ?? [];
                  const printingText = Array.isArray(printingList) ? printingList.join(', ') : '—';
                  ...
                  return (
                    <tr key={prod.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="p-3 text-[#891F1A] font-semibold">{prod.id}</td>
                      <td className="p-3 text-center">
                        <img
                          src={imgSrc}
                          alt="product"
                          ...
                          onError={(e) => ((e.currentTarget as HTMLImageElement).src = '/img1.jpg')}
                        />
                      </td>
                      <td className="p-3">{prod.name || prod.title || '—'}</td>
                      <td className="p-3 text-center text-red-600 font-bold">{prod.quantity}</td>
                      <td className="p-3 text-center text-green-700 font-bold">£{prod.price || 0}</td>
