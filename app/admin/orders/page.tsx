...
const BASE_BTN = "px-4 py-2 text-sm rounded-full border transition-all";
const STATUS_COLORS: Record<(typeof FILTERS)[number], string> = {
...
export default function AdminOrdersPage() {
  ...
  const handleSaveOrder = (order: any) => {
    ...
    const status = capitalizeStatus(String(order.status ?? "Pending"));
              ...
              {FILTERS.map((status) => {
                ...
                return (
                  <button
                    ...
                    className={`${BASE_BTN} ${STATUS_COLORS[status]} ${activeRing}`}
                  >
                    {status}
                  </button>
                );
