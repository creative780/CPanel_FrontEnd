...
export default function AdminOrdersPage() {
  ...
  const getStatusStyle = (status: string) => {
    const base = 'inline-flex items-center gap-2 text-xs font-semibold px-3 py-1 rounded-full';
    switch (status) {
      case 'Pending':
  ...
  const handleSaveOrder = (order: any) => {
    ...
    const items = typeof order.items === 'number' ? order.items : order.items_count ?? 0;
  ...
  return (
    <AdminAuthGuard>
        ...
        <div className="flex-1 px-6 py-8 bg-gray-50 min-h-screen">
          <div className="max-w-7xl mx-auto">
            <div className="mb-6 bg-gradient-to-r from-white via-[#f8f9fa] to-gray-100 p-6 rounded-2xl shadow-sm border border-gray-200 flex items-center justify-between">
              <div>
                <h1 className="text-3xl font-bold text-gray-800 flex items-center gap-2">🧾 Orders</h1>
                <p className="text-gray-500 mt-1 text-sm">View, manage, and track all customer orders in one place.</p>
              </div>
              <div className="flex items-center gap-4">
                <span className="text-gray-400 hidden md:block text-sm">Last updated: {new Date().toLocaleDateString()}</span>
                <button
            ...
            <div className="flex flex-wrap items-center gap-3 mb-6">
              {['All', 'Pending', 'Processing', 'Shipped', 'Completed'].map((status) => {
                ...
                const baseStyle = 'px-4 py-2 text-sm rounded-full border transition-all';
                const statusColors: { [key: string]: string } = {
                  All: 'border-gray-300 bg-white text-gray-700 hover:bg-gray-100',
                  Pending: 'border-yellow-300 bg-yellow-50 text-yellow-700 hover:bg-yellow-100',
                  Processing: 'border-blue-300 bg-blue-50 text-blue-700 hover:bg-blue-100',
                  Shipped: 'border-indigo-300 bg-indigo-50 text-indigo-700 hover:bg-indigo-100',
                  Completed: 'border-green-300 bg-green-50 text-green-700 hover:bg-green-100',
                };
                const activeRing = isActive ? 'ring-2 ring-offset-1 ring-[#891F1A]' : '';
                return (
                  <button
                    key={status}
            ...
            <div className="overflow-hidden bg-white rounded-2xl shadow-xl border border-gray-200">
              <table className="w-full table-auto text-sm">
                <thead className="bg-[#891F1A] text-white text-xs uppercase tracking-wide">
                  <tr>
                    <th className="p-4 text-left w-50">Order ID</th>
                    <th className="p-4 text-left w-50">Date</th>
                    <th className="p-4 text-left w-150">Customer</th>
                    <th className="p-4 text-center w-50">Items</th>
                    <th className="p-4 text-right w-50">Total</th>
                    <th className="p-4 text-center w-50">Status</th>
                    <th className="p-4 text-center w-50">Action</th>
                  </tr>
                </thead>
                  ...
                  {orders
                    ...
                    .map((order) => (
                      <tr key={order.id} className="hover:bg-gray-50 transition">
                        <td className="p-4 font-semibold text-[#891F1A]">{order.id}</td>
                        <td className="p-4 text-gray-600">{order.date}</td>
                        <td className="p-4">{order.customer}</td>
                        <td className="p-4 text-center">{order.items}</td>
                        <td className="p-4 text-right font-bold text-green-700">${order.total.toFixed(2)}</td>
                        <td className="p-4 text-center">
                          <select
                            value={order.status}
                            onChange={(e) => handleStatusChange(order.id, e.target.value as Order['status'])}
                            className={`${getStatusStyle(order.status)} bg-white cursor-pointer`}
                          >
      ...
      {isModalOpen && <OrderForm onClose={() => setIsModalOpen(false)} onSave={handleSaveOrder} />}
