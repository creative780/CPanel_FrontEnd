...
// ⬇️ Add frontend key helper (minimal, safe)
...
export default function FirstCarouselPage() {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/api/first-carousel/`, withFrontendKey()) // ⬅️ add header
      .then(res => res.json())
      .then(data => {
    ...
    try {
      const uploadedImageData = await Promise.all(
        images.map(async (img) => {
          ...
          // ⛔️ Do not change this logic (kept exactly as requested)
          if (img.type === 'url') {
      ...
      const response = await fetch(`${API_BASE_URL}/api/first-carousel/`, withFrontendKey({ // ⬅️ add header
        method: 'POST',
