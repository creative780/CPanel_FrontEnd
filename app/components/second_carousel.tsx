...
export default function SecondCarousel() {
  ...
  // Clamp index if fewer images than before
  useEffect(() => {
    setCurrentIndex((prev) => Math.min(prev, maxIndex));
  }, [maxIndex]);

  // Fetch from backend only (no dummy fallback)
  useEffect(() => {
    const baseUrl = API_BASE_URL;
    fetchWithKey(`${baseUrl}/api/second-carousel/?_=${Date.now()}`)
      .then((res) => res.json())
      .then((data) => {
      ...
      .catch(() => {
        setCarouselData({ title: "", description: "", images: [] });
      });
  }, []);

  // Translate track on desktop
  useEffect(() => {
  ...
  const totalPages = hasImages
    ? Math.ceil(carouselData.images.length / itemsPerView)
    : 0;
  ...
  return (
    <section className="w-full py-7 px-1 md:px-0 flex flex-col items-center">
      <header className="text-center max-w-[800px] mb-8">
        {carouselData.title && (
          <h1 className="text-[#891F1A] text-[32px] font-bold">
            {carouselData.title}
          </h1>
        )}
      ...
      <div className="relative w-full max-w-screen-xl mb-6">
        <div className="overflow-x-auto md:overflow-hidden px-[6px] sm:px-[10px] md:px-[14px]">
          {hasImages ? (
                ...
                >
                  ...
                  <div className="py-2 flex justify-between items-start gap-2">
                    <div className="flex-1">
                      {item.title && (
                        <h3 className="text-sm font-semibold text-[#333] text-left">
                          {item.title}
                        </h3>
                      )}
                      {item.caption && (
            ...
            <div className="w-full flex items-center justify-center py-10 h-[0px]"></div>
          )}
        </div>
      </div>
