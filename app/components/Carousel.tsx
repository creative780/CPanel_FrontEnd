...
export default function FirstCarousel() {
  ...
  // A stricter slugify, ported from the Python version you shared
const slugify = (val: string, allowUnicode = false): string => {
  if (!val) return "";

  let value = val.toString();

  if (allowUnicode) {
    // Normalize unicode to NFC (closest to Python's NFKC)
    value = value.normalize("NFKC");
  } else {
    // Strip accents/diacritics by normalizing to NFD and dropping marks
    value = value.normalize("NFD").replace(/[\u0300-\u036f]/g, "");
  }

  // Lowercase, remove anything not alphanum/underscore/hyphen/space
  value = value.toLowerCase().replace(/[^\w\s-]/g, "");

  // Collapse whitespace/dashes into single dash, then trim leading/trailing
      ...
      .then((data) => {
        if (Array.isArray(data?.images)) {
          const newImages: CarouselImage[] = data.images.map((img: any, i: number) => {
            ...
            const computedSlug = preferredSlug || slugify(catName || img?.title || `Product ${i + 1}`);
