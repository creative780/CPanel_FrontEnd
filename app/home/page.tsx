...
export default function PrintingServicePage() {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/api/hero-banner/`, withFrontendKey())
      ...
      .then((data) => {
        ...
        const desktop = all.filter((img: any) => img.device_type === "desktop").map((img: any) => img.url);
        const mobile = all.filter((img: any) => img.device_type === "mobile").map((img: any) => img.url);
        const mid = Math.ceil(all.length / 2);

        setDesktopImages(desktop.length ? desktop : all.slice(0, mid).map((img: any) => img.url) || [fallbackImage]);
        setMobileImages(mobile.length ? mobile : all.slice(mid).map((img: any) => img.url) || [fallbackImage]);
      })
      .catch(() => {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/api/show-categories/`, withFrontendKey())
      .then((res) => res.json())
      .then((data) => {
        const visible = data.filter((category: any) => category.status === "visible");
  ...
  return (
    ...
    >
      ...
      <img height="250" src="/images/Banner3.jpg" alt="Banner Image" className="block bg-[#D9D9D9] w-full h-auto mx-auto" />

      {/* CATEGORIES */}
      <section className="sm:px-6 lg:px-10 py-8 m-0" aria-labelledby="categories-heading">
        {/* h2 → SemiBold (600) */}
        <h2 id="categories-heading" className="text-[#891F1A] text-2xl sm:text-3xl font-semibold text-center mb-6">
          Discover our categories
        </h2>

       <div className="grid grid-cols-2 sm:grid-cols-4 gap-7 mb-6 w-[100%] ">
        {categories.map((category: any) => {
          const formattedUrl = `/home/${category.name.toLowerCase().replace(/\s+/g, "-")}`;
          return (
            <article key={category.id} className="contents">
              <Link
                href={formattedUrl}
                className="flex flex-col items-start cursor-pointer hover:scale-105 transition-transform duration-300"
              ...
              >
                ...
                />
                <h3 className="mt-2 text-md font-medium text-[#333] text-left">
                  {category.name}
                </h3>
              </Link>
            </article>
      ...
      <img height="250" src="/images/Banner2.jpg" alt="Banner Image" className="block bg-[#D9D9D9]  w-full h-auto" />
      ...
      {/* CTA */}
      <section className="flex flex-col lg:flex-row items-center sm:px-6 lg:px-12 xl:px-10 py-12 bg-white" aria-labelledby="cta-heading">

        <div className="flex-1">
          <p className="text-[#837E8C] text-sm font-normal mb-2">Call To Action</p>
          {/* h2 → SemiBold (600) */}
          <h2 id="cta-heading" className="text-[#0E0E0E] text-3xl sm:text-4xl font-semibold leading-tight mb-4">
          ...
          {/* Callback Form */}
          <form onSubmit={handleSubmit} className="mt-10 space-y-6 max-w-xl" aria-label="Request a callback">
            <div>
        ...
        <aside aria-label="Decorative placeholder" className="w-full mr-[10px] sm:w-[500px] h-[600px] bg-[#8B8491] rounded-xl" />
      </section>

      <div className="w-full bg-white h-[100px]" aria-hidden="true"></div>

      {/* CONTACT INFO */}
      <section className="bg-[#FAFAFA] px-4 sm:px-6 lg:px-24 py-8" aria-labelledby="contact-heading">
